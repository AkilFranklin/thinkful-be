...
async function averageRating(req, res, next) {
  // your solution here
  const average = await restaurantsService.averageRating();
  res.json({ data: { average_rating: Number(average.avg) } });
}
...
async function count(req, res, next) {
  // your solution here
  const num = await restaurantsService.count();
  res.json({ data: { count: parseInt(num.count, 10) }});
}
...
async function readHighestRating(req, res, next) {
  // your solution here
  const rating = await restaurantsService.readHighestRating();
  res.json({ data: { max_rating: rating["max"] }});
}
